...
document.getElementById("generarResumenFinal").addEventListener("click", function() {
    ...
    // Breakeven
    resumen += `<b>⚖️ Operaciones breakeven (${breakeven}):</b><br><div>`;
    listaBreakeven.forEach((op, i) => {
        resumen += `<div>#${i + 1}: ${op}$<br><br></div>`;
    });
    ...
    resumen += `<b>📈 Margen neto de ganadas:</b> ${margenGanadas >= 0 ? "+" : ""}${margenGanadas.toFixed(2)}$<br><br>`;
    resumen += `<b>📉 Margen neto de perdidas:</b> -${Math.abs(margenPerdidas).toFixed(2)}$<br><br>`;
    let promedioSemanal = totalOperaciones / semanas;
    let diasOperables = semanas * 6; // si operas 5 días a la semana
    let promedioDiario = totalOperaciones / diasOperables;
    resumen += `<b>📊 Promedio semanal de operaciones:</b> ${promedioSemanal.toFixed(2)}<br><br>`;
    resumen += `<b>📊 Promedio diario de operaciones:</b> ${promedioDiario.toFixed(2)}<br><br>`;
    ...
    if (!isNaN(capitalInicialVal) && capitalInicialVal > 0) {
        ...
        resumen += `<b>📈 Crecimiento en dólares</b> Datos no ingresados.<br>`;
        resumen += `<b>📊 Crecimiento porcentual</b> Datos no ingresados.<br><br>`;
    }
    // ---------- fin del bloque ----------
    ...
    const comentariosTexto = document.getElementById("comentarios").value;
    const objetivosTexto = document.getElementById("objetivos").value;

    const resumenHTML = `
    <div class="comentarios-container">
        <div class="comentarios">
            <label class="label-resumen" >Comentarios adicionales del mes</label>
            <p>${comentariosTexto}</p>
        </div>
        <div class="objetivos">
            <label class="label-resumen" >Objetivos para el próximo mes</label>
            <p>${objetivosTexto}</p>
        </div>
    </div>
    `;
        // ✅ En vez de ponerlo aparte, lo concatenas al final de `resumen`
    resumen += resumenHTML;
